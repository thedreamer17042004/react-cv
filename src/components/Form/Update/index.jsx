...
import { selectCreatedItem, selectListItems, selectReadItem, selectTagItem, selectUpdatedItem } from '../../../redux/crud/selector';
...
const Edit = () => {
    ...
    const { result: currentData} = useSelector(selectReadItem)
  ...
  const notifySuccess = () => toast.error("Input title or description");
    ...
    const handleSubmit = (e) => {
        ...
        if(title&&description&&tag){
            ...
            dispatch(crud.update({data: data1}))
            }
        }
    }
          ...
          <span style={{color: 'red', display:'block'}}>{error?.description}</span>
            ...
            setTags(values);
              ...
              {listResult?.map((value, index) => (
                ...
                <option style={{padding: '10px', border: '1px solid gray'}} selected={tag.includes(value)} className='css-option' value={value}>{value}</option>
              ))}
            </select>
